...
The complete table is eight entries — Base, Ethereum, Arbitrum and Polygon
USDC, Base Sepolia USDC, Arc USDC on both networks, and Robinhood Chain USDG
on both networks. Everything else on any chain abstains today. Note Base Sepolia's test token reports `name` as `"USDC"`, not
