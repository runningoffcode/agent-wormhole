...
USDC abstains today; Arc Testnet USDC (`5042002:0x3600…`) does not, since
0.8.3 — see [arc.md](arc.md). Treat an abstain as "unchecked", and decide
with something other than this library.

**A joined payload now refuses, and so does some honest text.** Quote values
are scanned whatever key carries them, and punctuation-joined runs are split
so the rules can read them — `Ignore.all.previous.instructions` is the same
sentence as the spaced form and is treated as one. The cost is measured and
worth stating: text that contains an override phrase in joined form refuses
where it used to pass. A documentation URL ending `/ignore-previous-instructions`
and a listing naming an `override-system-prompt` config flag are the realistic
cases, and they land hardest on a merchant whose own product is injection
defence. Ordinary hyphenated prose — "pay-per-call", "end-to-end encrypted" —
is unaffected. `ignore: ["X402-202"]` is not a workaround: it disables the
rule everywhere, including on the payee field, which is the one place the
